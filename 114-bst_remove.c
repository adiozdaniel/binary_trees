...
/**
 * find_node - this finds a node with a given value
...
/**
 * replace_with_successor - this function replaces a node
 * with its in-order successor in a binary search tree.
 * @node: A pointer to the node in the binary search tree.
 *
 * Return: A pointer to the node's in-order successor node.
 */
...
/**
 * bst_remove - this function removes a node
 * with a given value in a binary search tree
 * @root: binary search tree
 * @value: value of the node
 *
 * Return: A pointer to the tree's root node, OR NULL
 */
