...
/**
 * enqueue_item_4 - this function adds an item to a queue!
...
/**
 * dequeue_item_4 - this function removes an item
 * from a queue.
 * @queue_h: A pointer to the queue's head!
 * @queue_t: A pointer to the queue's tail!
 * @n: A pointer to the queue's size value!
...
/**
 * get_last_heap_node - this function gets the last level
 * order node \
 * in a max binary heap tree.
 * @root: The root of the max binary heap tree.
 *
 * Return: A pointer to the last level order node, Or NULL.
 */
...
/**
 * swap_tree_node_with_parent_1 - this function Swaps
 * a node in a \
...
/**
 * heap_extract - this function extracts the root node
 * of a max binary heap tree
 * @root: A pointer to the address of the tree's root node.
 *
 * Return: The value of the extracted node, Or NULL.
 */
